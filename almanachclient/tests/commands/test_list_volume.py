...
class TestListVolumeCommand(base.TestCase):
    ...
    def setUp(self):
        ...
        self.command = ListVolumeCommand(self.app, self.app_args)

    def test_execute_command(self):
        self.args.tenant_id = 'some uuid'
        self.args.start = '2017-01-01'
        self.args.end = '2017-01-30'
        self.client.get_volumes.return_value = [{'entity_id': 'some uuid', 'project_id': 'tenant id'}]

        expected = (('Volume ID', 'Name', 'Start', 'End', 'Type', 'Size', 'Attachments'),
                    [('some uuid', None, None, None, None, None, None)])

        self.assertEqual(expected, self.command.take_action(self.args))

        self.client.get_volumes.assert_called_once_with(self.args.tenant_id,
